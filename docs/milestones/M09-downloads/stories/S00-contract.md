# S00 — Contrato da experiência — M09

Status: S00 concluída; EXPERIENCE.md revisada contra cobertura.

## Objetivo

Definir o percurso: Detalhes → baixar e escolher destino → fila → pausar → reabrir → retomar → cancelar preservando catálogo.
...
Documentar EXPERIENCE.md com campos, mensagens, rotas propostas (/downloads; escolha de destino; confirmação separada para apagar dados), transições e fixtures para fila vazia; queued; downloading; paused; complete; sem espaço; resume inválido; falha; cancelado. Mapear cada obrigação da cobertura e definir baseline/corpus quando houver budget.
