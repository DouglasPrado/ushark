# S00 — Contrato da experiência — M17

Status: concluída; EXPERIENCE.md define transições e limites.

## Objetivo

Definir o percurso: Biblioteca assinada → duplicar → novo nome → editar cópia → atualizar origem → comparar independência.
...
Documentar EXPERIENCE.md com campos, mensagens, rotas propostas (ação Duplicar; editor pessoal; coleções pessoais), transições e fixtures para copiando; cancelado; falhou; cópia pronta; origem indisponível; asset compartilhado. Mapear cada obrigação da cobertura e definir baseline/corpus quando houver budget.
