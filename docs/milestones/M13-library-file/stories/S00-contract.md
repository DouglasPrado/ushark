# S00 — Contrato da experiência — M13

Status: concluída; experiência em ../EXPERIENCE.md.

## Objetivo

Definir o percurso: Biblioteca → exportar → revisar pacote → importar em outro perfil → preview → confirmar → abrir offline.
...
Documentar EXPERIENCE.md com campos, mensagens, rotas propostas (exportação de biblioteca; import .tslib; preview/erros de compatibilidade), transições e fixtures para validando; inválido; incompatível; asset ausente; assinatura não suportada; cancelado; importando; conflito; sucesso. Mapear cada obrigação da cobertura e definir baseline/corpus quando houver budget.
